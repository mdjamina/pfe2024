...
def stanford_from_raw(raw_file):
    ...
    os.chdir(dir_path)
    # convert_command = 'java -cp "*;" edu.stanford.nlp.parser.lexparser.LexicalizedParser -outputFormat "penn"
    # edu/stanford/nlp/models/lexparser/englishPCFG.ser.gz '+ raw_file
    convert_command = ('java -cp "*;" edu.stanford.nlp.parser.lexparser.LexicalizedParser -outputFormat  "penn" '
                       '-tokenized  -escaper edu.stanford.nlp.process.PTBEscapingProcessor -sentences newline '
                       'edu/stanford/nlp/models/lexparser/englishPCFG.ser.gz ') + raw_file
...
def convert_to_dep_tree(constituency_tree_fn):
    ...
    convert_command = ("java -cp {0}/{1} edu.stanford.nlp.trees.EnglishGrammaticalStructure -treeFile {2} -basic "
                       "-conllx -makeCopulaHead -keepPunct ").format(
        dir_path, STANFORD_JAR, constituency_tree_fn)
    stream = os.popen(convert_command)
    return stream
...
def convert_to_dep_graph(constituency_tree_fn):
    dir_path = os.path.dirname(os.path.abspath(__file__))
    convert_command = ("java -cp {0}/{1} edu.stanford.nlp.trees.EnglishGrammaticalStructure -treeFile {2} -collapsed "
                       "-makeCopulaHead -keepPunct -originalDependencies").format(
        dir_path, STANFORD_JAR, constituency_tree_fn)
    stream = os.popen(convert_command)
    return stream
...
def create_dep_trees_from_stream(stream, wsjInfo_exists, collapsed=False):
    ...
    for line in stream:
        ...
        if init_flag:
            ...
            dep_trees_data = {0: []}
            dep_trees_nodes = {
                0: DepTree(pos="", word="ROOT", id=0, parent=None, parent_relation="", children=[], wsj_id=int(wsj_id),
                           sent_id=int(sent_id))}
            init_flag = False
            if wsjInfo_exists:
                continue
        ...
        if line != "":
            ...
            dep_trees_nodes[id] = DepTree(pos=node[3], word=node[1], id=node[0], parent=None, parent_relation=node[9],
                                          children=[], wsj_id=int(wsj_id), sent_id=int(sent_id))
...
def missing_children(treeNode, graphNode):
    ...
    ret = [Word(index=c.id, word=c.word) for c in treeNode.children if
           (c.parent_relation not in neighbors) or (c.id != neighbors[c.parent_relation][0].text[0].index) or (
...
def read_dep_graphs_file(constituency_tree_fn,
    ...
    for i, t in enumerate(trees):
        ...
        for node_id in nodesMap:
            ...
            if 'cc' in child_dic:
                conj_type = (" ".join([cc.word for cc in sorted(child_dic['cc'], key=lambda cc: cc.id)]),
...
if __name__ == "__main__":
    readNLTK()
# shell() ts = [t[0] for t in create_dep_trees_from_stream(open(r"C:\Users\user\Google_Drive\PHD\AI2
# Internship\project\gold_train.txt"), wsjInfo_exists=False)]
