...
from props.dependency_tree.definitions import domain_label, copular_verbs, \
    subject_dependencies, clausal_complements, \
    FIRST_ENTITY_LABEL, SECOND_ENTITY_LABEL, arguments_dependencies, determined_labels, SOURCE_LABEL, ARG_LABEL, \
    contractions
...
class GraphWrapper(digraph):
    ...
    def get_components(self):
        ...
        return {self.nodesMap[key.uid]: [self.nodesMap[v.uid] for v in value] for key, value in
                graph_components.iteritems()}
    ...
    def is_aux_edge(self, (src, dst)):
        ...
        if (not self.nodesMap[src].isPredicate) or (
        (label not in arguments_dependencies + clausal_complements)):  # and (not label.startswith("prep"))):
            return True
        return False
    ...
    def _merge(self):
        edges = find_edges(self, lambda (u, v): (self.edge_label((u, v)) in join_labels) or (
                    self.edge_label((u, v)) == "conj_and" and u.features.get("conjType", [""])[0] == '&'))
        for u, v in edges:
    ...
    def _fix(self):
        ...
        edges = find_edges(self,
                           lambda (u, v): (self.edge_label((u, v)) == "prep") and (len(self.neighbors(v)) == 1) and (
                                       "pobj" in v.neighbors()))
        if edges:
            for (u, v) in edges:
        ...
        for (u, v) in sorted(edges, key=lambda ((u, v)): u.minIndex()):
            ...
            candidates = [n for n in multi_get(neighbors, [rel for rel in neighbors if rel.startswith("prepc_")]) if
                          len(self.neighbors(n)) == 0]
    ...
    def do_conj(self):
        edges = find_edges(self, lambda ((u, v)): self.edge_label((u, v)).startswith(
        ...
        for conj1 in nodes:
            ...
            isModifier = (not bool([father for father in self.incidents(conj1) if
                                    not self.is_aux_edge((father.uid, conj1.uid))])) and bool(self.incidents(conj1))
            for rel in [rel for rel in curNeighbours if rel.startswith("conj_")]:
                    ...
                    if conj1.isPredicate:
                        for neighbor in self.neighbors(conj1):
                            if get_min_max_span(self, neighbor)[0] < curStartIndex:
                                for conj2 in curNeighbours[rel]:
                                    if (self.edge_label((conj1, neighbor)) == SOURCE_LABEL) or (
                                    not self.is_aux_edge((conj1.uid, neighbor.uid))):
                ...
                for conj2 in curNeighbours[rel]:
                    ...
                    conj1.surface_form = [w for w in conj1.surface_form if
                                          (w not in conj2.surface_form) and (w not in conj1.text)]
                    for w in conj1.text:
    ...
    def do_acomp(self):
        ...
        for predNode, acompNode in edges:
            ...
            if len(subjs) != 1:
                ...
                if (predNode.text[0].word in self.modalVerbs) or (
                        predNode.features.get("Lemma", "") in self.modalVerbs):
                    subj = subjs[0]
                    ...
                    if (len(self.neighbors(predNode)) == 0) and (len(predNode.text) == 1) and (
                            predNode.text[0].word in contractions):
    ...
    def _do_conditionals(self):
        ...
        edges = find_edges(self, lambda ((u, v)): (self.edge_label((u, v)) == "mark") and (
                    v.text[0].word.lower() in ["if", "while", "because", "although", "as", "once"]))
    ...
    def extract_entities(self):
        ret = find_nodes(graph=self, filterFunc=lambda node: (not node.isPredicate) and multi_get(node.incidents(),
                                                                                                  arguments_dependencies))
        return ret
            ...
            for indind, curInd in enumerate(filter(lambda x: x in curRange, appendix[key])):
                if PDF:
                    fout.write("\\hyperref[fig:{0}]{{{1}}}$\\;$\n".format(curInd,
                                                                          tex_escape(
                                                                              graphs[curInd].gTag.tree.report())))
                elif HTML:
