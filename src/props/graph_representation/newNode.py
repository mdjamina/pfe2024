...
from props.dependency_tree.definitions import definite_label
...
class Node:
        ...
        for feat, printFunc in PRINT_FEATURES:
            if feat in self.features:
                ...
                if self.features.get("Determiner", {"Value": ""})["Value"] == "no":
                    ...
                    continue
                ret += "<TR><TD>"
                ret += '<FONT POINT-SIZE="10">{0}</FONT>'.format(html.escape(str(printFunc(self.features[feat]))))
                ret += "</TD></TR>"
    ...
    def to_conll_like(self):
        return "\t".join([str(self.uid),
                          ";".join([w.to_conll_like() for w in self.text]), self.pos(),
                          str(int(bool(self.isPredicate))),
                          str(int(self.features.get("top", False))),
                          ";".join([",".join([self.gr.edge_label((father, self)), str(father.uid)]) for father in
                                    self.gr.incidents(self)])
                          ])
